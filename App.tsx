...
const App: React.FC = () => {
    ...
    try {
      ...
      if (analysis.error) {
        setError({
          ...
          message: analysis.errorMessage || "The clinical scanner couldn't quite resolve the data.",
          troubleshooting: [
            "Ensure the brand name and ingredient list are fully visible.",
            "Try to reduce glare from overhead lights on shiny packaging.",
            "Verify you have a stable internet connection for database grounding."
          ]
        });
      ...
      let errorMessage = "The Clinical AI engine encountered a network or server issue.";
      let troubleshooting: string[] = ["Check your local environment setup.", "Verify your API key status in Google AI Studio."];
      
      if (err.message === "MISSING_API_KEY") {
        errorTitle = "API Key Required";
        errorMessage = "A valid Gemini API key is missing from the system configuration.";
        troubleshooting = [
          "Check your .env file for API_KEY.",
          "Ensure the key is active and has billing enabled if required.",
          "Restart your local development server."
        ];
      } else if (err.message?.includes("429")) {
        errorTitle = "Capacity Reached";
        errorMessage = "The system is currently handling too many audit requests.";
        troubleshooting = ["Wait 60 seconds and try the scan again.", "Avoid rapid consecutive scans."];
      } else if (err.message?.includes("403") || err.message?.includes("401")) {
        errorTitle = "Access Denied";
        errorMessage = "The provided API key lacks permission for the Clinical AI model.";
        troubleshooting = ["Verify permissions in Google AI Studio.", "Ensure the gemini-3-flash-preview model is enabled for your key."];
      }

      setError({ title: errorTitle, message: errorMessage, troubleshooting });
      setLoadingState('error');
    }
  };
  ...
  const categoryConfigs = {
    [ProductCategory.FOOD]: { icon: 'fa-wheat-awn', label: 'Food & Nutrition', desc: 'Audit additives, neurotoxins, and UPF markers.', theme: 'emerald', accent: 'bg-emerald-500', light: 'bg-emerald-50' },
    [ProductCategory.COSMETICS]: { icon: 'fa-spray-can-sparkles', label: 'Cosmetics', desc: 'Scan for endocrine disruptors and skin toxins.', theme: 'rose', accent: 'bg-rose-500', light: 'bg-rose-50' },
    [ProductCategory.MEDICINE]: { icon: 'fa-microscope', label: 'Pharmaceutical', desc: 'Verify excipient purity and clinical side-effects.', theme: 'blue', accent: 'bg-blue-600', light: 'bg-blue-50' }
  };
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="hidden sm:flex flex-col items-end">
              ...
              <span className="text-[11px] font-bold text-slate-400">Clinical AI V3.2</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6">
        {loadingState === 'category_selection' && (
          <div className="pt-24 md:pt-32 text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <h1 className="text-6xl md:text-9xl font-black mb-6 tracking-tighter leading-[0.85] text-slate-900">is it <span className="text-emerald-600">shudh?</span></h1>
            <p className="text-slate-500 font-medium mb-20 text-xl md:text-2xl max-w-2xl mx-auto">Real-time clinical audit for hidden toxins in your daily essentials.</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {(Object.keys(categoryConfigs) as ProductCategory[]).map((cat) => (
                <button key={cat} onClick={() => { setSelectedCategory(cat); setLoadingState('idle'); }} className="group relative flex flex-col items-start p-10 md:p-12 bg-white rounded-[3rem] border border-slate-100 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] hover:-translate-y-3 transition-all duration-500 text-left overflow-hidden">
                  ...
                  <div className={`w-20 h-20 rounded-[1.75rem] mb-10 flex items-center justify-center text-4xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 shadow-sm ${cat === ProductCategory.FOOD ? 'bg-emerald-50 text-emerald-600' : cat === ProductCategory.COSMETICS ? 'bg-rose-50 text-rose-500' : 'bg-blue-50 text-blue-600'}`}>
          ...
          <div className="pt-24 flex flex-col items-center animate-in fade-in zoom-in-95 duration-700">
            <div className="w-full max-w-2xl bg-white rounded-[4rem] p-10 md:p-16 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] relative border border-slate-50">
              <div className="mb-14 text-center">
                <div className={`inline-flex items-center gap-3 px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-[0.2em] mb-6 shadow-sm ${CATEGORY_THEMES[selectedCategory].bg} ${CATEGORY_THEMES[selectedCategory].text} border ${CATEGORY_THEMES[selectedCategory].border}`}>
                  <i className={`fa-solid ${categoryConfigs[selectedCategory].icon}`}></i> Audit: {categoryConfigs[selectedCategory].label}
                </div>
              ...
              <div className="space-y-6">
                <button onClick={() => setLoadingState('camera')} className={`w-full p-10 bg-slate-950 hover:bg-black text-white rounded-[3rem] flex items-center justify-between transition-all active:scale-[0.98] shadow-2xl group relative overflow-hidden`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="relative bg-slate-50 hover:bg-slate-100 transition-all p-10 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-4 group cursor-pointer shadow-sm">
                    ...
                    <div className="w-14 h-14 rounded-2xl bg-white flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform"><i className="fa-solid fa-file-arrow-up text-slate-400 text-xl"></i></div>
                  ...
                  <div className="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-100 flex flex-col justify-center gap-2 group focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all shadow-sm">
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); handleAnalysis(undefined, undefined, nameInput); }} className="flex items-center gap-4">
                      <input type="text" placeholder="Product name..." value={nameInput} onChange={(e) => setNameInput(e.target.value)} className="bg-transparent text-xl font-bold outline-none w-full placeholder:text-slate-300" />
        ...
        {loadingState === 'error' && error && (
          <div className="max-w-2xl mx-auto mt-24 mb-32 animate-in fade-in slide-in-from-bottom-4 duration-500 no-print">
            <div className="bg-white rounded-[4rem] border border-slate-100 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.1)] p-12 md:p-16 relative overflow-hidden">
               <div className="absolute top-0 right-0 p-12 opacity-5">
                 <i className="fa-solid fa-triangle-exclamation text-8xl text-rose-500"></i>
               </div>
               
               <div className="flex items-center gap-4 mb-8">
                 <div className="w-16 h-16 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center text-2xl shadow-inner">
                   <i className="fa-solid fa-shield-xmark"></i>
                 </div>
                 <div>
                   <h2 className="text-[10px] font-black uppercase text-rose-400 tracking-[0.4em] mb-1">Status: Failed</h2>
                   <h3 className="text-3xl font-black text-slate-900 tracking-tight">{error.title}</h3>
                 </div>
               </div>

               <p className="text-slate-500 font-medium text-xl mb-12 leading-relaxed italic border-l-4 border-rose-100 pl-6">
                 "{error.message}"
               </p>

               <div className="bg-slate-50 rounded-[2.5rem] p-10 mb-12">
                 <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] mb-6 flex items-center gap-2">
                   <i className="fa-solid fa-wrench"></i> Troubleshooting Protocol
                 </h4>
                 <ul className="space-y-4">
                   {error.troubleshooting.map((step, i) => (
                     <li key={i} className="flex items-start gap-4 text-slate-700 font-semibold">
                       <span className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-[10px] text-slate-400 shadow-sm border border-slate-100 flex-shrink-0 mt-0.5">{i+1}</span>
                       {step}
                     </li>
                   ))}
                 </ul>
               ...
               <div className="flex flex-col sm:flex-row gap-4">
                 <button onClick={reset} className="flex-1 py-6 bg-slate-950 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl hover:bg-black active:scale-95 transition-all">
                   Restart Lab
                 </button>
                 <button onClick={() => window.location.reload()} className="flex-1 py-6 bg-white text-slate-400 border border-slate-100 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] hover:bg-slate-50 transition-all">
                   Refresh Engine
                 </button>
               </div>
            </div>
